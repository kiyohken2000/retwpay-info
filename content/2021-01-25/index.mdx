...
AppStoreにkenmo readerのアップデートを提出したら却下された話と、Archive機能を少し変更した話を書きます。
...
まとめて取得機能は、「ストレージから登録したサイトのURLをロードして`fetch`を繰り返す」という実装になっています。
...
ローディングスピナーを撤去しただけのバージョンを再提出しても良かったのですが、追加したアーカイブ機能で修正したい箇所があったのでバージョン**1.0.5**としてビルドしなおすことにしました。

## アーカイブ機能の改善

<Link to="/blog/2021-01-24">前回</Link>追加した、記事をアーカイブする機能ですが、アーカイブ記事の一覧画面から記事表示画面に遷移したときは保存ボタンが表示されなという動きにしていました。
...
記事を読み終わった後に一覧画面に戻ってからチェックマークをタップするという使い方になるため操作の手数が多いのが気になりました。

---
title: Linkランキング機能を改善しました
tags: [    ]
date: 2021-02-26
path: blog/2021-02-26
cover: ./img.png
excerpt: Improved the Link ranking.
keywords:
    - 開発
    - Develop
    - React Native
...
というようにLike数を格納するフィールドを用意して、取り出すときは`20210226000`以上の投稿を最大20件取得するようにします。

ゼロ埋め用に1000をかけてますが、アプリの構造とかユーザー数的にLike数が1000を超える投稿はないだろうという判断です。

実際には日付ではなくて、**24時間以内**を取り出したいので`new Date().getTime()`を使ってミリ秒単位の時間にしました。
...
```javascript
onValueChanged() {
	let before24Hour = ( new Date().getTime() - (24 * 3600 * 1000) ) * 1000
	this.dbh
		.ref(`gallery/`)
...
```
`orderByChild`メソッドで**likeRank**フィールドをソートして**24時間前の時刻**を`startAt`メソッドで足きりすると、過去24時間の投稿がLike数順でソートされます。

Realtime Databaseでは昇順でソートされるので、Like数が多いものが後ろに来ます。なので`limitToLast`メソッドで、最後の20件を取り出します。

これで**過去24時間の投稿をLike数順に最大20件**取り出せました。
